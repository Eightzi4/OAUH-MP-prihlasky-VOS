...
@section('content')
    <div
        class="w-full max-w-md text-center bg-white/80 backdrop-blur-xl p-10 rounded-3xl shadow-2xl border border-white/60 ring-1 ring-black/5 relative overflow-hidden">

        <div class="mb-8 text-center relative z-10">
            <h1 class="text-2xl font-bold text-gray-900 mb-2">Administrace</h1>
            <p class="text-gray-500 text-sm">Přihlášení pro studijní oddělení</p>
        </div>

        <form action="{{ route('admin.login.post') }}" method="POST" class="space-y-6 text-left relative z-10">
            @csrf

            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">E-mail
                    administrátora</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span class="material-symbols-rounded text-gray-400 text-[20px]">alternate_email</span>
                    </div>
                    <input type="email" name="email" required autofocus placeholder="jan.novak@example.com"
            ...
            <div class="pt-4">
                <button type="submit"
                    class="group relative flex items-center justify-center px-8 py-4 rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer w-full">
                    <div class="absolute inset-0 topo-bg opacity-50 transition-opacity duration-300"></div>
                    <div
                        class="absolute inset-0 bg-white/60 backdrop-blur-[2px] group-hover:backdrop-blur-[4px] transition-all duration-300">
                    </div>
