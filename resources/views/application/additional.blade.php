...
@section('form-content')
    ...
    <form id="main-form" action="{{ route('application.storeStep3', $application->id) }}" method="POST"
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/60 p-8 ring-1 ring-black/5 mb-6">
            ...
            <textarea name="specific_needs" rows="3" placeholder="Zde popište své požadavky..."
                class="w-full py-3 px-4 border border-gray-200 rounded-xl bg-white/50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-school-primary focus:border-school-primary transition-all shadow-sm">{{ old('specific_needs', $application->specific_needs) }}</textarea>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/60 p-8 ring-1 ring-black/5 mb-6">
            ...
            <textarea name="note" rows="3" placeholder="Vaše poznámka..."
                class="w-full py-3 px-4 border border-gray-200 rounded-xl bg-white/50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-school-primary focus:border-school-primary transition-all shadow-sm">{{ old('note', $application->note) }}</textarea>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/60 p-8 ring-1 ring-black/5 mb-8"
            ...
            @error('other_files.*')
                <div class="flex items-center gap-1 mt-2 ml-1 text-school-warning">
                ...
                <template x-for="file in selectedFiles" :key="file.id">
                    <div class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl shadow-sm">
                        <div class="flex items-center gap-3 overflow-hidden">
                            ...
                            <template x-if="!file.previewUrl">
                                <div
                                    class="h-10 w-10 bg-red-50 rounded-lg flex items-center justify-center text-school-primary flex-shrink-0">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-gray-500">Připraveno k nahrání &bull; <span
                    ...
                    <div class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl shadow-sm">
                        <a href="{{ asset('storage/' . $file->disk_path) }}" target="_blank"
                            ...
                            @if (str_starts_with($file->mime_type, 'image/'))
                                <div
                                    class="h-10 w-10 rounded-lg overflow-hidden border border-gray-100 flex-shrink-0 bg-gray-100">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-green-600">Uloženo &bull; {{ round($file->size / 1024) }} KB</p>
                            </div>
                        </a>
                        <button form="delete-file-{{ $file->id }}"
                            class="text-gray-400 hover:text-red-500 transition-colors p-2" title="Odstranit">
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-sm border border-white/60 p-4 ring-1 ring-black/5">
            <div class="flex justify-between items-center">
                <button type="button" onclick="switchStep('{{ route('application.step2', $application->id) }}')"
