...
<div class="w-full max-w-md text-center bg-white/80 backdrop-blur-xl p-10 rounded-3xl shadow-2xl border border-white/60 ring-1 ring-black/5">
        ...
        <div class="flex flex-col gap-4 mt-8">
            <button type="submit" class="group relative w-full flex items-center justify-center px-8 py-4 rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer">
                ...
                <span class="relative z-10 text-gray-900 font-bold text-lg flex items-center drop-shadow-sm">
                    Přihlásit se
                    <span class="material-symbols-rounded ml-3 text-[20px] text-gray-600 group-hover:text-school-primary transition-transform duration-300 group-hover:translate-x-1">login</span>
                </span>
            </button>
        </div>
    </form>
    ...
    <form action="{{ route('auth.send-link') }}" method="POST" class="mt-8 border-t border-gray-200/60 pt-6">
        ...
        <button type="submit" class="text-sm font-bold text-school-primary hover:text-school-hover hover:underline transition-colors bg-transparent border-none cursor-pointer w-full text-center">
